...
class EvolutionaryOptimizer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self,
                 ...
                 fs_cv=3,
                 keep_originals=None  # None: libre; True: originales obligatorias; False: originales también seleccionables
                 ):
        # tiempo
    ...
    def fit(self, X, y):
        ...
        while (time.time() - start_time) < gp_budget:
            ...
            # mejora suficiente
            if best_fit_gen < self.best_fitness_ - self.early_stop_min_improv * max(1.0, abs(self.best_fitness_)):
    ...
    def transform(self, X):
        if not self.best_trees_:
            raise ValueError("No entrenado (best_trees_ vacío).")
        if hasattr(X, 'values'):
            X = X.values
    ...
    def _run_feature_selection_ga(self, X_aug, y, time_budget):
        ...
        for _ in range(self.fs_population_size):
            ...
            # originales y sintetizadas de base
    ...
    def _fs_fitness(self, X_aug, y, mask):
        ...
        if np.any(np.std(X_sub, axis=0) < 1e-12):
            return 1e8, None
        model = LinearRegression()
        scores = cross_val_score(model, X_sub, y, cv=self.fs_cv,
                                 scoring='neg_mean_absolute_error', n_jobs=-1)
...
if __name__ == "__main__":
    ...
    gp_optimizer = EvolutionaryOptimizer(
        ...
        fs_l1_penalty=1e-4,
        fs_cv=3,
        keep_originals=None  # pon True para obligar originales; False para permitir quitarlas
    )
