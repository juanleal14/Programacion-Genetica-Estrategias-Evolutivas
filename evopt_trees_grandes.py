...
class EvolutionaryOptimizer(BaseEstimator, TransformerMixin):
    ...
    def _generate_tree(self, max_depth):
        """Genera un árbol aleatorio con sesgo hacia complejidad."""
        # FOMENTAR ÁRBOLES MÁS COMPLEJOS
        terminal_prob = 0.1 if max_depth > 3 else 0.2 if max_depth > 1 else 0.8
        
        if max_depth <= 1 or random.random() < terminal_prob:
            if random.random() < 0.8:  # Feature
                return GPNode(random.randint(0, self.n_features_in_ - 1), node_type='terminal')
            else:  # Constante
                return GPNode(random.uniform(-3, 3), node_type='terminal')
        
        else:  # Función - FAVORECER BINARIAS
            if random.random() < 0.7:  # 70% binarias
                binary_funcs = [f for f, arity in self.functions.items() if arity == 2]
                func_name = random.choice(binary_funcs)
            else:  # 30% unarias
                unary_funcs = [f for f, arity in self.functions.items() if arity == 1]
                func_name = random.choice(unary_funcs)
    ...
    def _evaluate_individual(self, trees, X, y):
        ...
        try:
            ...
            # Penalización por complejidad REDUCIDA
            complexity = sum(tree.size() for tree in trees)
            penalty = 0.0005 * complexity  # Menos penalización
            
            # BONIFICACIÓN por diversidad estructural
            diversity_bonus = 0
            for tree in trees:
                if tree.depth() > 2:  # Árboles profundos
                    diversity_bonus -= 0.01
                if tree.size() > 5:   # Árboles grandes
                    diversity_bonus -= 0.005
            
            penalty += diversity_bonus
    ...
    def _crossover_trees(self, parent1, parent2):
        ...
        # Cruce dentro de árboles individuales MÁS FRECUENTE
        for i in range(min(len(child1), len(child2))):
            if random.random() < 0.5:  # Más cruce
                child1[i], child2[i] = self._crossover_single_tree(child1[i], child2[i])
        
        return child1, child2
    
    def _crossover_single_tree(self, tree1, tree2):
        """Cruce entre dos árboles individuales MÁS AGRESIVO."""
        # MÚLTIPLES tipos de cruce
        if random.random() < 0.5:  # Cruce de subárboles
        ...
        elif random.random() < 0.3:  # Cruce de funciones
            if tree1.node_type == 'function' and tree2.node_type == 'function':
                # Intercambiar funciones manteniendo aridad compatible
                func1_arity = self.functions[tree1.value]
                func2_arity = self.functions[tree2.value]
                if func1_arity == func2_arity:
                    tree1.value, tree2.value = tree2.value, tree1.value
    ...
    def _mutate_trees(self, trees):
        ...
        # MÁS reemplazo de árboles completos
        if random.random() < 0.15:
            idx = random.randint(0, len(mutated) - 1)
            # FAVORECER árboles más profundos
            new_depth = random.randint(3, self.max_depth)
            mutated[idx] = self._generate_tree(max_depth=new_depth)
    ...
    def _mutate_single_tree(self, tree):
        ...
        # Mutar recursivamente hijos MÁS AGRESIVO
        for i, child in enumerate(mutated.children):
            if random.random() < 0.3:
                mutated.children[i] = self._mutate_single_tree(child)
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*60}")
    print(f"EVALUACIÓN INDIVIDUAL DE LOS 8 ÁRBOLES")
    print(f"{'='*60}")
